...
def conv_forward_naive(x, w, b, conv_param):
    ...
    H_out = np.int(((H + 2*pad - HH) / stride) + 1)
    W_out = np.int(((W + 2*pad - WW) / stride) + 1)
    ...
    for nn in range(N):  # For each image in the input batch.
        for ff in range(F):  # For each filter in w
            for jj in range(0, H_out):  # For each output pixel height
                for ii in range(0, W_out):  # For each output pixel width
                    out[nn, ff, jj, ii] = \
...
def max_pool_forward_naive(x, pool_param):
    ...
    # Naive convolution loop.
    for nn in range(N):  # For each image in the input batch.
        for cc in range(C):  # For each filter in w
            for jj in range(0,H_out,):  # For each output pixel height
                for ii in range(0, W_out):  # For each output pixel width
...
def max_pool_backward_naive(dout, cache):
    ...
    for nn in range(N):
        for ff in range(F):
            for jj in range(dout_H):
                for ii in range(dout_W):
                  a = x[nn, ff, jj*stride:jj*stride+pH, ii*stride:ii*stride+pW]
                  ind = np.unravel_index(np.argmax(a, axis=None), a.shape)
                  dx[nn,ff,jj*stride+ind[0], ii*stride+ind[1]] = dout[nn,ff,jj,ii]
