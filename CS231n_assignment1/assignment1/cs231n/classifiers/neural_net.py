...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    shift_scores = scores - np.max(scores, 1)[..., np.newaxis]
    correct_class_scores = np.choose(y, shift_scores.T)

    loss = -correct_class_scores + np.log(np.sum(np.exp(shift_scores), 1))
    loss = np.sum(loss)
    ...
    # Calculate dscores.
    dScore = np.exp(shift_scores)/ np.sum(np.exp(shift_scores), axis=1)[..., np.newaxis]
    dScore[range(len(X)),y] = dScore[range(len(X)),y] - 1
    dScore /= len(X)

    # Backprop dScore to calculate dW, then average and add regularisation.
    dW2 = np.dot(L1.T, dScore)
    dW2 += 2*reg*W2
    grads['W2'] = dW2
